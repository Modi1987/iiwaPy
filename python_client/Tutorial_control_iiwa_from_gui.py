...
class App:
    def __init__(self):
        ...
        for i in range(7):
            ...
            Entry(upperFrame, justify="center", textvariable=self.jText[i]).grid(
                row=j, column=1
            )

            if i == 0:
                Label(upperFrame, text="    IP:").grid(row=j + 1, column=3)
                IPtxt = Entry(upperFrame, textvariable=self.IP_of_robot).grid(
                    row=j + 1, column=4
                )
                self.IP_of_robot.set("172.31.1.147")
            elif i == 1:
                Label(upperFrame, text=" State:").grid(row=j + 1, column=3)
                IPtxt = Entry(upperFrame, textvariable=self.stateMessage).grid(
                    row=j + 1, column=4
                )
        ...
        Button(
            ...
            text="Connect",
            fg="red",
            command=self.connectFun,
            height=buttonHeight,
            width=buttonWidth,
        ).grid(row=rowNum, column=4)
        Button(
            upperFrame,
            text="turn off server",
            fg="red",
            command=self.disconnectFun,
            height=buttonHeight,
            width=buttonWidth,
        ).grid(row=rowNum, column=5)
        rowNum = rowNum + 1
        Button(
            upperFrame,
            text="add new point",
        ...
        rowNum = rowNum + 1
        Button(
            upperFrame, text="Go home", fg="red", height=buttonHeight, width=buttonWidth
        ).grid(row=rowNum, column=5)
        rowNum = rowNum + 1
        Button(
            upperFrame, text="About", fg="red", height=buttonHeight, width=buttonWidth
        ).grid(row=rowNum, column=5)
